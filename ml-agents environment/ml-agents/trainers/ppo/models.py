...
class PPOModel(LearningModel):
    def __init__(self, brain, lr=1e-4, h_size=128, epsilon=0.2, beta=1e-3, max_step=5e6,
                 ...
                 curiosity_strength=0.01, curiosity_enc_size=128, scope='Model', seed=0,with_heuristics=False):
        """
    ...
    def create_curiosity_encoders(self):
        ...
        if self.vis_obs_size > 0:
            ...
            fused_next_visual_pre = []

            # Create the encoder ops for current and next visual input. Not that these encoders are siamese.
            encoded_visual = self.create_visual_observation_encoder(self.fused_visual_in, self.curiosity_enc_size,
            ...
            for i in range(self.vis_obs_size):
                ...
                if i == 0:
                    if self.use_segmentation: # go through network
                        img_out  = self.build_encoder_decoder(next_visual_input)
                        ...
                        fused_next_visual_pre.append(one_hot_image_down)
                    else:
                        print("###############     curiosity     ###################")
                        one_hot_image = self.modify_to_one_hot(next_visual_input)
                        print("first input for fusing: ground truth segmented image")
                        print("(",one_hot_image,")")
                        print("#####################################################")
                        fused_next_visual_pre.append(one_hot_image)
                        
                else:
                    print("###############     curiosity     ###################")
                    print("second input for fusing: ground truth segmented image")
                    print("(",next_visual_input,")")
                    print("#####################################################")
                    fused_next_visual_pre.append(next_visual_input)


            self.fused_next_visual_in = self.fuse_type_1(fused_next_visual_pre)
            encoded_next_visual = self.create_visual_observation_encoder(self.fused_next_visual_in,
    ...
    def create_forward_model(self, encoded_state, encoded_next_state):
        ...
        pred_next_state = tf.layers.dense(hidden, self.curiosity_enc_size * (self.fused_vis_obs_size + int(self.vec_obs_size > 0)),
                                          activation=None)
